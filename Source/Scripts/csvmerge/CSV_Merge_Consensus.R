...
csvmerge <- function(myArgs){
  ...
  exh$Allele2_Units[which(exh$Chr == "Y")] <- exh$Allele1_Units[which(exh$Chr == "Y")]
  ...
  names(genes) <- c("Call_ID", "Region", "Gene")
  "###############     CSV MERGING COMPLETE     ###############"
  return(list(compiledf, genes))
  }
merge_output <- csvmerge(myArgs)
con_input <- data.frame(merge_output[1])
genes <- data.frame(merge_output[2])
  
csvconsensus <- function(myArgs, input, genes){
  csv <- select(input, c("Call_ID", 'Sample_ID', "Chr", 'Start', "Ref.ExH", "Allele1.ExH", "Allele2.ExH", "Ref.Gang", "Allele1.Gang", "Allele2.Gang"))
  ...
  consensusdf <- merge(consensusdf, genes, by = "Call_ID")
  write.table(consensusdf, file=out_path, quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")    
  }
csvconsensus(myArgs, con_input, genes)
